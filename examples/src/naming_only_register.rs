...
use nacos_rust_client::client::{ HostInfo, AuthInfo, ServerEndpointInfo };
...
fn register(service_name:&str,group_name:&str,ips:&Vec<String>,port:u32,conn_manage:&Addr<ConnManage>) {
    log::info!("register,{},{}",service_name,group_name);
    for ip in ips {
        let instance = Instance::new_simple(&ip,port,service_name,group_name);
        //注册
...
where T:Actor<Context = actix::Context<T>> + Send
 {
