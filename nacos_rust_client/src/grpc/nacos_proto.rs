...
pub mod request_stream_server {
    ...
    #[async_trait]
    pub trait RequestStream: std::marker::Send + std::marker::Sync + 'static {
        /// Server streaming response type for the requestStream method.
        type requestStreamStream: tonic::codegen::tokio_stream::Stream<
    ...
    #[derive(Debug)]
    pub struct RequestStreamServer<T> {
        inner: Arc<T>,
                ...
                "/RequestStream/requestStream" => {
                        ...
                        for requestStreamSvc<T>
                    {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as RequestStream>::request_stream(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for RequestStreamServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "RequestStream";
    impl<T> tonic::server::NamedService for RequestStreamServer<T> {
